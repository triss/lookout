// Line crossings with stills. Full-screen camera + user-drawn line.
...
const settings = {
  ...
  name: "line-capture", viewType: "other",
...
async function recordCrossing(tr, dir, t) {
  ...
  } else if (settings.name && settings.name !== "line-capture") {
...
function download(text, type, ext) {
  ...
  const name = `lookout-line-capture-${new Date().toISOString().replace(/[:.]/g, "-")}.${ext}`;
...
$("btnJson").addEventListener("click", async () => {
  try {
    ...
    const session = {
      schema: "lookout.line-capture.session.v1",
    ...
    download(JSON.stringify({
      schema: "lookout.line-capture.export.v1",
